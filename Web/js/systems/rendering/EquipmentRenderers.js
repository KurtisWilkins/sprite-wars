...
/** Clean sparkle highlight — small white pixel cross */
function _eqDrawCleanSparkle(ctx, cx, cy, size, color = '#ffffff') {
    ctx.save();
    ctx.fillStyle = color;
    ctx.globalAlpha = 0.7;
    const s = size || 2;
...
/** Clean glow indicator — simple filled rectangle with low opacity */
function _eqDrawCleanGlow(ctx, cx, cy, radius, color = '#ffffff') {
    ctx.save();
    ctx.fillStyle = color;
    ctx.globalAlpha = 0.2;
...
export function drawWeaponByConfig(ctx, wx, wy, dir, config, colors) {
    ...
    // Clean cel-shaded: subtle glow indicator for magical weapons
    if (gl) {
        _eqDrawCleanGlow(ctx, wx + bW / 2, wy - bLen / 2, Math.floor(bLen * 0.4), gl);
        _eqDrawCleanSparkle(ctx, wx + bW / 2, wy - bLen + 2, 2, gl);
    }
    ...
    // Clean cel-shaded: particle indicators as small sparkle highlights
    if (config.hasParticles && config.particleColor) {
        const seed = (wx * 7 + wy * 13) % 5;
        _eqDrawCleanSparkle(ctx, wx - 4 + seed, wy - bLen + 4, 2, config.particleColor);
        _eqDrawCleanSparkle(ctx, wx + 6 - seed, wy - bLen + 10, 2, config.particleColor);
        if (bLen > 20) _eqDrawCleanSparkle(ctx, wx + seed - 2, wy - bLen + 16, 2, config.particleColor);
    }
}
...
export function drawHelmetByConfig(ctx, x, y, w, h, config, colors) {
    ...
    // Clean cel-shaded: subtle glow indicator for magical helmets
    if (gl) {
        _eqDrawCleanGlow(ctx, x + w / 2, y + ht / 2, Math.floor(Math.max(w, ht) * 0.5), gl);
    }
...
export function drawChestByConfig(ctx, x, y, w, h, config, colors) {
    ...
    // Clean cel-shaded: subtle glow indicator for magical chest armor
    if (gl) {
        _eqDrawCleanGlow(ctx, x + w / 2, y + h / 2, Math.floor(Math.max(w, h) * 0.4), gl);
    }
...
export function drawLegsArmorByConfig(ctx, legX, legY, legW, legH, config) {
    ...
    if (gl) {
        _eqDrawCleanGlow(ctx, legX + legW / 2, legY + legH / 2, Math.floor(Math.max(legW, legH) * 0.4), gl);
    }
